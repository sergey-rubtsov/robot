...
central_axis_diameter = 3.0
d_shaft = 1.3  # D-flat on shaft
...
# Axial Ball Bearing F3-8M 3x8x3.5mm
axial_bearing_internal_diameter = 3.0
axial_bearing_external_diameter = 8.0
axial_bearing_length = 3.5

inner_roller_back_length = 4.5
cover_length = 8.0
...
screw_length = cycloid_length * 2 + ring_bearing_length

screw_type = 'ISO7046'
screw_diameter = 'M3'
screw_length = str(screw_length)
screw_tap = 'simple'
...
class Screw(object):
  ...
  def getScrew(self, ST_text, ND_text, l):
    self.rThread = False
  ...
  def createScrew(self, ST_text, ND_text, NL_text, threadType):
    # FreeCAD.Console.PrintMessage(ST_text + " " + ND_text + " " + NL_text + " " + threadType + "\n")
    if threadType == 'real':
      self.rThread = True
    else:
      self.rThread = False
  ...
  def makeIso7046(self, SType ='ISO7046', ThreadType ='M6', l=25.0):
    ...
    if (SType == 'ISO10642') or (SType == 'ISO14582'):
      ...
      if (SType == 'ISO7046'):
        ...
        edge1 = Part.makeLine(Pnt0,Pnt1)  # make flat head
        edge2 = Part.makeLine(Pnt1,Pnt2)
        # TODO: remake it, it should be optional
        # recess, recessShell = self.makeCross_H3(cT, mH, ht)
  ...
  def makeScrewTap(self, ThreadType ='M3', l=25.0):
...
def get_nuts(count=10, diameter=45.0, z=0.0, thread_type='M3', length=3.0):
    hexagons = null_set()
    p, c, da, dw, e, m, mw, s = iso4035def[thread_type]
    for r in range(count):
      hexagon = make_hexagonal_prism(s, length)
      hexagon.rotate(Base.Vector(0, 0, 0), Base.Vector(0, 0, 1), 30.0)
      hexagon.translate(Base.Vector(diameter / 2, 0, z - length))
      hexagon.rotate(Base.Vector(0, 0, 0), Base.Vector(0, 0, 1), r * 360 / count)
      hexagons = hexagons.fuse(hexagon)
    return hexagons
...
def make_hexagon_wire(minimal_diameter: float) -> Part.Wire:
  r"""Make hexagon wire.
  Got here: https://gist.github.com/gbroques/d52f31fe629392017d254b4d71b91516
  ::
        _____     ┯
       /     \    |
      /       \   | minimal diameter
...
def contracted_cycloid_blank(pin_diameter=6.0, _cycloid_length=4.0):
    ...
    for theta in range(circle + 3):
        xNew = ((d + delta) / 2) * math.cos(theta * math.pi / circle_pi)
        yNew = ((d + delta) / 2) * math.sin(theta * math.pi / circle_pi)
        phi = d * theta / delta
        xNew += e * math.cos((theta + phi) * math.pi / circle_pi)
        yNew += e * math.sin((theta + phi) * math.pi / circle_pi)
        if theta >= 2:
...
def contracted_cycloid(dd,
    ...
    for r in range(holes):
        ...
        cc = cc.cut(roller)
    centre = Part.makeCylinder(dc / 2, _cycloid_length * 2 + 2, Base.Vector(0, 0, -1), Base.Vector(0, 0, 1))
    cc = cc.cut(centre)
...
def eccentric(diameter=10.0,
              bearing_length=4.0,
              _central_axis_diameter=3.0,
              _d_shaft=1.3):
    ecc = Part.makeCylinder(diameter / 2,
                            bearing_length,
                            Base.Vector(0, 0, 0),
                            Base.Vector(0, 0, 1))
    ecc.translate(Base.Vector(-e, 0, 0))
    ecc2 = Part.makeCylinder(diameter / 2,
                             bearing_length,
                             Base.Vector(0, 0, 0),
                             Base.Vector(0, 0, 1))
    ecc2.translate(Base.Vector(-e, 0, bearing_length))
    ecc2 = ecc2.rotate(Base.Vector(0, 0, 0), Base.Vector(0, 0, 1), 180)
    axis = Part.makeCylinder(_central_axis_diameter / 2, bearing_length * 2 * 1.1 + 10,
                             Base.Vector(0, 0, -10),
                             Base.Vector(0, 0, 10))
    d_shaft_cut = Part.makeBox(_central_axis_diameter, _central_axis_diameter, bearing_length * 2 * 1.11 + 10,
                               Base.Vector(-_central_axis_diameter / 2, -_central_axis_diameter / 2, -10),
                               Base.Vector(0, 0, 10))
    d_shaft_cut.translate(Base.Vector(-_d_shaft + _central_axis_diameter, 0, 0))
    axis = axis.cut(d_shaft_cut)
    axis.rotate(Base.Vector(0, 0, 0), Base.Vector(0, 0, 1), 90)
    ecc = ecc.fuse(ecc2)
    ecc = ecc.cut(axis)
    ecc.removeSplitter()
    return ecc
...
def forth_roller(_holes=3,
    ...
    central_axis = Part.makeCylinder(_small_bearing_internal_diameter / 2,
                                     ...
                                     Base.Vector(0, 0, -50),
                                     Base.Vector(0, 0, 50))
    inner_roller = inner_roller.cut(central_axis)
...
def get_pins(count=10, diameter=45.0, pin_diameter=3.0, length=3.0):
    pins = null_set()
    for r in range(count):
...
def ring_cover(external_diameter=45.0,
               stop_length=2.0,
               pin_external_diameter=6.0,
               length=8.0):
    r3 = Part.makeCylinder(external_diameter / 2,
                           length,
                           Base.Vector(0, 0, 0),
                           Base.Vector(0, 0, 1))
    r3 = r3.cut(
         Part.makeCylinder(external_diameter / 2 - pin_external_diameter / 2,
                           length,
                           Base.Vector(0, 0, 0),
                           Base.Vector(0, 0, 1)))
    r3 = r3.cut(
         Part.makeCylinder(external_diameter / 2 - stop_length / 2,
                           stop_length,
                           Base.Vector(0, 0, 0),
                           Base.Vector(0, 0, 1)))
    return r3
...
def get_motor_screws(_length=10.0, z=0.0):
    stator_screw_long_distance = 19.0
    stator_screw_short_distance = 16.0
    _stator_screw_type = 'ISO7046'
    _stator_screw_diameter = 'M3'

    long_stator_screw0 = Screw().getScrew(_stator_screw_type, _stator_screw_diameter, _length)
    long_stator_screw0.translate(Base.Vector(stator_screw_long_distance / 2, 0, z))
    long_stator_screw1 = Screw().getScrew(_stator_screw_type, _stator_screw_diameter, _length)
    long_stator_screw1.translate(Base.Vector(-stator_screw_long_distance / 2, 0, z))
    long_stator_screw2 = Screw().getScrew(_stator_screw_type, _stator_screw_diameter, _length)
    long_stator_screw2.translate(Base.Vector(0, stator_screw_short_distance / 2, z))
    long_stator_screw3 = Screw().getScrew(_stator_screw_type, _stator_screw_diameter, _length)
    long_stator_screw3.translate(Base.Vector(0, -stator_screw_short_distance / 2, z))
    return long_stator_screw0.fuse(long_stator_screw1).fuse(long_stator_screw2).fuse(long_stator_screw3)


def get_motor(diameter=27.0, length=25.0, z=0.0, _central_axis_diameter=3.0):
    _motor = Part.makeCylinder(diameter / 2,
                               length,
                               Base.Vector(0, 0, 0),
                               Base.Vector(0, 0, 1))
    axis = Part.makeCylinder(_central_axis_diameter / 2,
                             length * 2.2,
                             Base.Vector(0, 0, -0.5),
                             Base.Vector(0, 0, 0.5))
    wires = Part.makeCylinder(_central_axis_diameter * 3 / 4,
                              length,
                              Base.Vector(0, 0, 0),
                              Base.Vector(1, 1, 0))
    wires.translate(Base.Vector(0, 0, length - 2.5))
    _motor = _motor.fuse(axis)
    _motor = _motor.fuse(wires)
    _motor.translate(Base.Vector(0, 0, z))
    return _motor
...
def get_motor_cover(diameter=27.0, length=30.0, _central_axis_diameter=5.0, _thickness=4.0, _gap=1.5, z=0.0):
    ...
    _motor_obj_cover.Circumradius = (diameter + _thickness * 2) / 2
    _motor_obj_cover.Height = length + _thickness * 2 + _gap * 2
    _motor_obj_cover.FirstAngle = 0.0
    _motor_obj_cover.SecondAngle = 0.0
    _motor_obj_cover.Placement = App.Placement(App.Vector(0.0, 0.0, -(_gap * 2)), App.Rotation(0.0, 0.0, 0.0))
    doc.recompute()
    _motor_cover = _motor_obj_cover.Shape

    _axis.translate(Base.Vector(0, 0, -1000 / 2))
...
eccentric = eccentric(eccentric_bearing_internal_diameter,
                      ...
                      d_shaft)
cycloid = contracted_cycloid(dd,
                             dc,
                             cycloid_length,
                             -e,
                             0,
                             0)
contr_cycloid = contracted_cycloid(dd,
                                   dc,
                                   cycloid_length,
                                   e,
                                   cycloid_length,
...
axial_bearing = Part.makeCylinder(axial_bearing_external_diameter / 2 + gap / 4,
                                  ...
                                  Base.Vector(0, 0, 0),
                                  Base.Vector(0, 0, 1))
axial_bearing.translate(Base.Vector(0, 0, -axial_bearing_length / 2 - (inner_roller_back_length + 0.5)))
...
roller_disc_back = roller_disc_back.cut(axial_bearing)

roller_nuts_position = -inner_roller_back_length + nut_length - 0.1
roller_nuts = get_nuts(holes, dd, roller_nuts_position, screw_diameter, nut_length)
roller_disc_back = roller_disc_back.cut(roller_nuts)
...
motor_cover = get_motor_cover(motor_diameter, motor_length, 6.0, 4.0, motor_cover_gap, motor_position - thickness - motor_cover_gap / 2)
motor_cover = motor_cover.cut(motor_screws)
encoder_nuts = get_nuts(4, 17 * sqrt(2), motor_position - thickness - motor_cover_gap / 2, screw_diameter, nut_length)
encoder_nuts.rotate(Base.Vector(0, 0, 0), Base.Vector(0, 0, 1), 45.0)
motor_cover = motor_cover.cut(encoder_nuts)
motor_cover = motor_cover.cut(motor)
